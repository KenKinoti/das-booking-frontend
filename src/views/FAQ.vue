...
const ARTICLES = [
  ...
  { id: 'currencies', topic: 'start', q: 'Which currencies are supported?', a: ['23 currencies across major, Asia-Pacific, Middle East and African markets — including AUD, USD, EUR, GBP, NZD, SGD, AED, ZAR, NGN and KES. Choose the country your business is based in under Settings → Business details: its currency becomes the default for new invoices, POS sales, bookings, events, bills and pay runs (you can pick another). Existing records keep their currency, and invoices, quotes, employees and pay runs can each use their own.'], link: { to: '/settings', label: 'Open settings' } },
  { id: 'deposits', topic: 'start', q: 'A client already paid part of an invoice — how do I record it?', a: ['In the invoice editor switch on “Already paid / deposit” and enter the amount, date, method (cash, card, bank transfer, mobile money such as M-Pesa…) and reference. Saving records it as a payment: the invoice becomes partially paid and shows “Deposit received” and the balance due. You can do the same when converting a quote to an invoice.'], link: { to: '/invoices/new', label: 'New invoice' } },
